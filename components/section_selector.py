...
def render_scoping_summary():
    """Render a summary of active scoping activities in weekly report."""
    st.subheader("🎯 Active Scoping Activities")
    
    # Load scoping activities
    from components.scoping_process import load_scoping_activities
    activities = load_scoping_activities()
    
    if not activities:
        st.info("No active scoping activities.")
        return
    
    # Filter to show only active ones
    active_activities = [a for a in activities if a.get('current_phase') not in ['Contract Signed', 'Project Initiation']]
    
    if not active_activities:
        st.info("No active scoping activities this week.")
        return
    
    for activity in active_activities[:3]:  # Show top 3
        customer = activity.get('customer_name', 'Unknown')
        project = activity.get('project_title', 'Untitled')
        phase = activity.get('current_phase', 'Not Started')
        priority = activity.get('priority', 'Medium')
        
        priority_icon = {'High': '🔴', 'Medium': '🟡', 'Low': '🟢'}.get(priority, '🟡')
        
        st.write(f"{priority_icon} **{customer}** - {project}")
        st.write(f"   Current Phase: {phase}")
        
        next_steps = activity.get('next_steps', '')
        if next_steps:
            st.write(f"   Next Steps: {next_steps[:100]}{'...' if len(next_steps) > 100 else ''}")
    
    if len(active_activities) > 3:
        st.write(f"... and {len(active_activities) - 3} more activities")